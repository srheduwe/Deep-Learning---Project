...
class SchNetAC(AbstractActorCritic):
    def __init__(
        ...
        # Critic 2
        # create embedding for critic 
        self.embedding_fn_c = spk.representation.SchNet(n_atom_basis=self.num_afeats)

        # MLP beta for bag representation for critic
        self.phi_beta_c = MLP(
            input_dim=self.num_zs,
            output_dims=(network_width, self.num_latent_beta),
        )
    ...
    # atomic tensors for critic
    def make_atomic_tensors_c(
        self, observations: List[ObservationType]
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for i, observation in enumerate(observations):
            ...
            bag_tuple = [count for atomic_num, count in formula]
            if len(atoms) > 0:
                features[i, :len(atoms), :] = self.embedding_fn_c(self.converter(atoms))
                focus_mask[i, :len(atoms)] = 1
                focus_mask_next[i, :len(atoms) + 1] = 1
            else:
                focus_mask[i, :1] = 1  # focus null-atom
    ...
    def step(self, observations: List[ObservationType], actions: Optional[np.ndarray] = None) -> dict:
        ...
        atomic_feats_c, focus_mask_c, focus_mask_next_c, element_count_c, action_mask_c = self.make_atomic_tensors_c(observations)

        # Latent bag
        latent_bag_c = self.phi_beta_c(element_count_c)

        # Critic
        weights_c = focus_mask_c.unsqueeze(-1).float()  # n_obs x n_atoms x 1
        weights_c = weights_c.transpose(1, 2)  # n_obs x 1 x n_atoms
        sum_atomic_feats_c = (weights_c @ atomic_feats_c).squeeze(1)  # n_obs x n_afeats
        # mean_atomic_feats = sum_atomic_feats / torch.sum(focus_mask, dim=-1, keepdim=True)
        v_V = self.critic_c(torch.cat([sum_atomic_feats_c, latent_bag_c], dim=-1))
