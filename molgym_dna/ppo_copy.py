...
from molgym.buffer_copy import DynamicPPOBuffer
...
def compute_loss_value(
    ...
    # Value loss
    vf_loss = vf_coef * (pred['v_V'] - ret).pow(2).mean()
...
def compute_loss_distillation(
    ...
    # Total loss
    loss = (pred['v'] - pred['v_V']).pow(2).mean()  + dis_coef*sum(rel_entr(old_logp, new_logp))
...
def train(
    ...
    for i in range(max_num_steps):
        ...
        for batch_indices in batch_generator:
            ...
            policy_loss, policy_info = compute_loss_clip(ac,
                                                  data=data_batch,
                                                  clip_ratio=clip_ratio,
                                                  entropy_coef=entropy_coef,
                                                  device=device)

            value_loss, value_info = compute_loss_value(ac,
                                                  data=data_batch,
                                                  vf_coef=entropy_coef,
                                                  device=device)

            dis_loss, dis_info = compute_loss_distillation(ac,
                                                  data=data_batch,
                                                  vf_loss=value_loss,
                                                  dis_coef=dis_coef,
                                                  device=device)

            batch_loss = policy_loss + dis_loss + value_loss
